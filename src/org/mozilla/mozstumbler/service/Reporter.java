...
import org.mozilla.mozstumbler.service.cellscanner.CellInfo;
...
final class Reporter extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(WifiScanner.ACTION_WIFIS_SCANNED)) {
            ...
            receivedGpsMessage(intent); // FIXME this calls reportCollectedLocation but more calls follow below
        }
        ...
        if (currentPosition != null && Math.abs(time - currentPosition.getTime()) > REPORTER_WINDOW) {
            reportCollectedLocation(); // FIXME calls reportCollectedLocation again?
        }

        if (mBundle != null &&
            (mBundle.getWifiData().size() > WIFI_COUNT_WATERMARK ||
             mBundle.getCellData().size() > CELLS_COUNT_WATERMARK)) {
            reportCollectedLocation(); // FIXME calls reportCollectedLocation yet again??
        }
    }
